...
class RelicWidget():
    ...
    def update_relic_widgets(self):
        relics = get_all_relic_from_file()
        for relic in sorted(relics.keys()):
            tier = get_relic_tier_from_name(relic)
            reward = []
            for elem in relics[relic]['Intact']:
                relic_name = elem['name_' + OptionsHandler.get_option("Language", str)]
                rarity = get_relic_rarity_from_percent(elem['rarity'], "Intact")
                data = (relic_name, rarity)
                reward.append(data)
